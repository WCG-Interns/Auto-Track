...
import { FaCar, FaUser, FaCalendarAlt, FaSave, FaTimes, FaEye, FaEdit, FaPlus, FaFileAlt, FaShieldAlt, FaCertificate, FaLeaf, FaMoneyBillWave } from 'react-icons/fa';
    ...
    try {
      if (isAdd) {
        ...
        const response = await axios.put(
          `https://auto-track-server.onrender.com/${formData._id}`,
          formData,
          {
            headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
          }
            ...
            <div className="flex items-center">
              ...
              <div>
                <h2 className="text-2xl font-bold text-white">
                  {isView ? 'View Vehicle Details' : isEdit ? 'Edit Vehicle' : 'Add New Vehicle'}
                </h2>
                <p className="text-gray-300 mt-1">
                  {isView ? 'Vehicle information and documentation status' : isEdit ? 'Update vehicle information and documents' : 'Register a new vehicle in the system'}
                </p>
              </div>
            </div>
            <button
                  ...
                  return (
                    <div key={field}>
                      ...
                      <input
                        ...
                        disabled={isView}
                        className={`w-full px-4 py-3 bg-slate-600/50 border border-slate-500/50 rounded-xl text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500/50 transition-all duration-200 ${
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={handleChange}
                    disabled={isView || loadingUsers}
                    className={`w-full px-4 py-3 bg-slate-600/50 border border-slate-500/50 rounded-xl text-white focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500/50 transition-all duration-200 ${
                      isView ? 'cursor-not-allowed opacity-75' : 'hover:bg-slate-600/70'
                    }`}
                  >
                    ...
                    {users.map((user) => (
                      <option key={user._id} value={user._id} className="bg-slate-700">
