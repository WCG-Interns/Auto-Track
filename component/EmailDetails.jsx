...
const EmailList = ({ token }) => {
  ...
  useEffect(() => {
    const fetchEmails = async () => {
      try {
        const res = await axios.get('https://auto-track-server.onrender.com/api/emails', {
          headers: {
  ...
  const Deletehandler = async (emailId) => {
    ...
    try {
      await axios.delete(`https://auto-track-server.onrender.com/api/emails/${emailId}`, {
        headers: {
          Authorization: `Bearer ${token}`,
  ...
  const Edithandler = async (emailId, newEmail) => {
    try {
      const res = await axios.put(`https://auto-track-server.onrender.com/api/emails/${emailId}`, 
        { email: newEmail }, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
